...
function App() {
  ...
  const selectedCustomers: { id: string }[] = data.selected || [];
  ...
  const handleSubmit = async () => {
    ...
    try {
      const customerIds = selectedCustomers.map((c: { id: string }) => c.id);
  ...
  return (
    <AdminAction
      primaryAction={
        <Button onPress={handleSubmit} disabled={isSubmitting}>
          {isSubmitting ? "Creating..." : "Create Gift Cards"}
        </Button>
      }
      secondaryAction={
    ...
    >
      <BlockStack gap="base">
        <Text fontWeight="bold">Create Gift Cards for Selected Customers</Text>
        <Text>
          {customerCount} customer{customerCount !== 1 ? "s" : ""} selected.
        </Text>

        {customerCount === 50 && (
          <Banner tone="warning">
            Shopify limits bulk selections to 50 customers. For more, use the Customer
            Segment action.
          </Banner>
        )}
